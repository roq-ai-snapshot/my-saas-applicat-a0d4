...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.contact_information?.length > 0) {
      const create_contact_information = body.contact_information;
      body.contact_information = {
        create: create_contact_information,
      };
    } else {
      delete body.contact_information;
    }
    ...
    if (body?.opening_hour?.length > 0) {
      const create_opening_hour = body.opening_hour;
      body.opening_hour = {
        create: create_opening_hour,
      };
    } else {
      delete body.opening_hour;
    }
